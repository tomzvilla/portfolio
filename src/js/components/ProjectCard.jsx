...
const ProjectCard = ({project}) => {
    ...
    //     const storedAnimationRan = localStorage.getItem(`animationRan-project-card`);
  ...
  return (
    <div className='flex flex-col'>
        <div className='flex flex-col m-auto items-center justify-center z-10 mt-24 lg:flex-row xl:gap-28'>
            {/* card content */}
            <Reveal transitionDuration={0.5} identifier={`project-${project.title}`}>
                <div ref={ref} className='flex flex-col items-center md:items-start max-w-[700px] lg:max-w-[600px] justify-center text-white space-y-7 z-10'>
            ...
            >
                <div className='w-[300px] sm:w-[350px] rounded-xl aspect-[16/9] lg:ml-10 ease-linear transition-all overflow-hidden'>
                    <img loading="lazy" className='w-full h-full object-cover transition-transform duration-300 transform-gpu hover:scale-110' src={getImageUrl(project.thumbnail)} alt="" />
                </div>
                <div className='flex flex-row justify-center lg:ml-10 mt-3 gap-5'>
                    {project.projectGithub && <a href={project.projectGithub} target='_blank' className='hover:-rotate-12'>
                    ...
                    {project.projectShowcase && <a href={project.projectShowcase} target='_blank' className='hover:-rotate-12'>
